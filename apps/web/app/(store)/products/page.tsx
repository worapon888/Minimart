...
// ✅ ดึง image แบบ type-safe (รองรับ thumbnail / images / imageUrl)
const getImageSrc = (p: Product): string | null => {
  // รองรับ backend field: imageUrl โดยไม่ใช้ any
...
function ProductsPageContent() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const json = await fetchJSON<unknown>(`${API_BASE}/products`, {
        ...
        if (Array.isArray(json)) {
          data = json;
        } else if (isProductsResponse(json)) {
          data = json.data;
        } else if (isObject(json)) {
          // ไม่ใช้ any: เช็ค key ทีละตัวแบบ safe
        ...
        console.log("✅ /products normalized count:", data.length);
        if (data[0]) console.log("✅ first product:", data[0]);

        // ถ้าคุณอยาก “strict” จริง ๆ: ทำ isProduct guard แล้วกรองก่อน set ก็ได้
  ...
  /** ---------- GSAP list animate (กัน target not found) ---------- */
  useLayoutEffect(() => {
    cardsRef.current = [];
