...
import type { ApiResponse } from "../../../../../../packages/shared/types/api";
...
// ✅ type guard ขั้นต่ำของ Product (กันพังแบบ practical)
const isProduct = (x: unknown): x is Product => {
  if (!isObject(x)) return false;
  if (!hasKey(x, "id") || !hasKey(x, "title")) return false;
...
const isApiResponse = <T,>(
  ...
  if (!isObject(x)) return false;
  if (!hasKey(x, "ok") || typeof x.ok !== "boolean") return false;
...
// ✅ normalize product จากหลายรูปแบบ response โดยไม่ใช้ any
const normalizeProduct = (json: unknown): Product | null => {
  // 1) product ตรง ๆ
  if (isProduct(json)) return json;

  // 2) { ok, data: product }
  if (isApiResponse<Product>(json, isProduct)) return json.data;

  // 3) { product: product } / { item: product } / { data: product } (แบบหลวม)
  if (isObject(json)) {
    if (hasKey(json, "product") && isProduct(json.product)) return json.product;
    if (hasKey(json, "item") && isProduct(json.item)) return json.item;
    if (hasKey(json, "data") && isProduct(json.data)) return json.data;
  }

  return null;
};

// ✅ ดึงรูปจากหลาย field แบบ type-safe
const getImageSrc = (p: Product): string | null => {
  // ถ้า Product type คุณมี imageUrl จริง แนะนำให้ใส่ใน type เลย
  // แต่ถ้ายังไม่อยากแก้ type ใช้ in-operator แบบปลอด any
  if (
    isObject(p) &&
    "imageUrl" in p &&
...
export default function ProductDetailPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const json = await fetchJSON(`${base}/products/${id}`, {
          signal: controller.signal,
          cache: "no-store",
        });

        const p = normalizeProduct(json);
        setProduct(p);
      } catch (err: unknown) {
  ...
  const hasSize = useMemo(() => {
    const cat =
      typeof product?.category === "string"
        ...
        : "";
    return !!cat && HAS_SIZE_CATEGORIES.includes(cat);
  }, [product?.category]);
  ...
  const price = getPriceUSD(product);
