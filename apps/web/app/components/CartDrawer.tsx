...
import { CartItemLike } from "../../../../packages/shared/types/product";
...
export default function CartDrawer({
  ...
  const items = useMemo(
    () => ((state.items as unknown as CartItemLike[]) ?? []),
    [state.items],
  );

  // ✅ กันพัง: สรุปราคาให้เป็น number เสมอ (ไม่พึ่ง it.price โดยตรง)
  const safePrice = useCallback((it: CartItemLike) => getPriceUSD(it) ?? 0, []);

  const totalPrice = useMemo(
    () => items.reduce((sum, it) => sum + safePrice(it) * it.quantity, 0),
    [items, safePrice],
  );
          ...
          {items.length === 0 ? (
            ...
            <div className="space-y-4">
              {items.map((item) => {
                ...
                const unitPrice = safePrice(item);
